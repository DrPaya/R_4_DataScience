...
na.omit(read.csv("Respiration_2012_combined_demo.csv")) ##Real example that works with existing data with added NA to challenge expression
...
```{r}
data <- na.omit(read.csv("hw1_data.csv")) ## assigning the imported csv file to the string "data", where NA is ommitted for the entire table
...
table <- read.csv("hw1_data.csv")  ##Imports the data frame
table[is.na(table)] <- ""   ##identifies anything that is NA, and the expression assigns ""

table <- na.omit(read.csv("hw1_data.csv"))  #Imports the data table/data frame
...
table <- na.omit(read.csv("hw1_data.csv")) ##assigns table to the data frame
...
thing <- read.csv("name") ##for removing na without removing the complete row. 
thing[is.na(thing)] <- ""
...
combinedata <- function( id = 1:x) {  ##This is a function for combining multiple csv's for data analysis, where the number of combined files can be defined in the second argument. 
  fileslist <- list.files(".../R Tutorials/Programming_in_R/specdata", full.names = TRUE) ##Not actual file path.
...
x <- strptime(datestring, "%B %d, %Y %H:%M")  ## strptime function works here- passed a format string. % Bhe month, %d the day, %Y 4-digit year, and then hours and minutes. 
x
...
## The with() looks for with( data, expr) where the data is a data.frame environment, and expr is the expression, whjich in this case was tapply)
...
```{r}
ls() ##Shows you all of the variables in your "workspace", which is everything that was assigned 
class() ## tells you whether you have a vector, data.frame, matrix, etc...
dim()  ## shows you how many 1) rows, and 2) columns you have in your data.frame. The dimensions.
nrow()  ##Shows the number of rows in a data.frame
ncol() ##shows the number of columns in a data.frame
object.size() ##shows you the number of bytes being used by this data.frame
names()  ##gives you a return of the column names of the data.frame
head(x, 10)  ##shows the first few rows of a data.frame so that not all observations are returned. The second expression can be used to choose the number of rows you wish to see, in this case, 10 rows.
tail()  ##can be used in the same way as head, except it looks at the bottom of the data.frame, 
summary() ##gives you a brief overview of each column's data. 
table(data.frame$column_name)  ##In this expression, you can get summary data from a column containing categorical variables. 
...
##Row sub-setting by desired values - two column example, but can limit to one. 
      iris[iris$Sepal.Length %in% c("4.5", "4.6", "4.7") & iris$Petal.Length %in% c("1.4"),]
...
```{r}
str(mtcars)
name(mtcars)  ##returns column names
...
mtcars <- rename(mtcars, weight = wt, horse = hp) ##RENAME function in dplyr, where the new column name is first, followed by "= existing column name"
head(mtcars)
...
head(education, n = 20)
      names(gdp) 
      gdp2 <- rename(gdp, Countrycode = X, country = X.2, milDollars = X.3, rank = Gross.domestic.product.2012) ##This is useful for a rename, but not for the merge function. Headding returns the original column names, x etc...
      names(education)
...
grep("") 
table(grepl("integer, number of character", data.frame$column)) ##will present theu number of instances the integer appears within the column of choosing. 
...
download.file(fileURL, destfile = "./data/idahoHousing.csv", method = "curl")  ##curl is essential because https is a protected file. 
list.files("./data")
dateDownloaded <- date()
dateDownloaded
    idaho <- read.csv(".../R Tutorials/Programming_in_R/data/idahoHousing.csv") ##Not actual file path. 
    head(idaho)
    #s <- split(idaho, idaho$VAL)  ##splitting the dataset idaho by the value - a good way to get summary data. 
    
    sapply(s, function(x) sum(x[, "VAL"], na.rm = TRUE)) ##careful here- it sums all of the codes, which means that you have to divide by the code value to get count. 
...
best <- function(state, result, removeNA = TRUE) {
    outcome <- read.csv("outcome-of-care-measures.csv", colClasses = "character") ##assigning the data.frame the title outcome
...
```{r}
  ...
  rankhospital <- function(state, outcome, rank, removeNA = TRUE) {
    datafile <- read.csv("outcome-of-care-measures.csv", colClasses = "character") ##assigning the data.frame the title outcome
...
list.files("./run_analysis/")
datapath <-file.path("/Users/payashome/Documents/FMDtRH/R Studio/R Tutorials/run_analysis", "UCI HAR Dataset")
